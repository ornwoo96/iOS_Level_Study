...
**상속(Inheritance)** 과 **프로토콜(Protocol)** 은 객체 지향 프로그래밍에서 코드의 재사용과 구조화를 돕는 개념이지만, 두 가지 방식에는 중요한 차이가 있습니다.

### 상속 (Inheritance)

상속은 기존 클래스를 기반으로 새로운 클래스를 정의하는 방법입니다. 상속을 사용하면 기존 클래스(부모 클래스)의 속성과 메서드를 자식 클래스가 물려받아 사용할 수 있으며, 필요에 따라 이를 **재정의(Override)** 할 수 있습니다.

<br>

#### 특징:
- 클래스 간의 관계: 상속은 클래스 간의 is-a 관계를 나타냅니다. 예를 들어, Bird 클래스가 Animal 클래스를 상속받으면, Bird는 Animal의 일종이라고 할 수 있습니다.
- 단일 상속: Swift에서는 클래스가 단일 상속만 지원하므로, 하나의 클래스는 오직 하나의 부모 클래스만 가질 수 있습니다.
- 코드 재사용: 부모 클래스에 정의된 속성, 메서드 등을 자식 클래스에서 재사용할 수 있어 코드의 중복을 줄일 수 있습니다.
...
위 예시에서 Dog 클래스는 Animal 클래스를 상속받아 makeSound 메서드를 재정의합니다.

<br>

### 프로토콜 (Protocol)

프로토콜은 특정 기능이나 속성을 정의하는 청사진(규약)입니다. 프로토콜을 통해 타입이 가져야 할 속성이나 메서드를 미리 명세해 둘 수 있으며, 프로토콜을 채택한 타입이 해당 요구사항을 구현해야 합니다.

#### 특징:
- 다중 채택 가능: 클래스, 구조체, 열거형은 여러 개의 프로토콜을 동시에 채택할 수 있습니다.
- 구현의 강제: 프로토콜은 구현할 메서드와 속성을 명시할 수 있으며, 이를 채택하는 타입에 반드시 구현하도록 강제합니다.
- 유연한 타입 구성: 클래스뿐만 아니라 구조체와 열거형도 프로토콜을 채택할 수 있어, 다양한 타입에 기능을 확장할 수 있습니다.
...
airplane.fly() // 출력: Airplane flying
```

Flyable 프로토콜을 채택한 Bird 클래스와 Airplane 구조체는 모두 fly() 메서드를 구현해야 합니다.

<br>

### 상속과 프로토콜의 차이점 요약
<img src="https://github.com/user-attachments/assets/f49adbb1-3412-45d2-9c25-737e4cf32980">

- is-a 관계는 하위 클래스가 상위 클래스의 일종임을 나타내며, 하위 클래스가 상위 클래스의 모든 속성, 메서드를 물려받아 같은 유형으로 간주될 수 있는 관계입니다.
- can-do 관계는 여러 타입이 특정 기능을 제공할 수 있도록 규정하여, 어떤 타입이든 해당 프로토콜을 채택하면 동일한 기능을 수행할 수 있습니다.

<br>

### 언제 상속과 프로토콜을 사용할까?
#### 상속: 
클래스 간에 명확한 계층 구조가 필요할 때, 코드의 재사용을 위해 사용합니다. 예를 들어, Animal 클래스에서 Dog, Cat 클래스를 정의할 때처럼 is-a 관계일 때 적합합니다.

<br>

#### 프로토콜: 
여러 타입에서 공통 기능을 요구할 때, 특히 다중 채택이 필요한 상황에서 사용합니다. 예를 들어, Flyable, Drivable 등의 행동을 정의하는 경우처럼 특정 동작을 여러 타입에 걸쳐 정의하고 싶을 때 유용합니다.
...
UI 테스트와 단위 테스트는 모두 애플리케이션의 품질을 유지하는 데 중요한 역할을 하지만, 테스트하는 범위와 목적이 다릅니다.

<br>

### 1. 사용자 인터페이스(UI) 테스트

UI 테스트는 애플리케이션의 사용자 인터페이스가 예상대로 동작하는지를 검증하는 테스트입니다. 화면에 나타나는 요소들이 올바르게 동작하고, 사용자가 직접 수행할 액션들이 의도한 결과를 가져오는지 확인하는 데 중점을 둡니다.

- 주요 목적: 사용자 인터페이스와 기능이 사용자가 기대하는 대로 동작하는지 확인
- 테스트 범위: 애플리케이션의 전체 흐름이나 주요 사용자 플로우
- 실행 환경: 실제 장치나 시뮬레이터에서 테스트가 이루어짐
- 종류: 시나리오 테스트, 화면 전환 테스트, 버튼 및 액션 동작 확인 등

#### 예시 코드 : swift에서 UI Test 작성

```swift
// 앱이 특정 버튼을 클릭했을 때 올바른 화면으로 이동하는지 테스트하는 예제입니다.
func testButtonNavigation() {
    ...
    XCTAssertTrue(nextButton.exists)  // 버튼이 존재하는지 확인
    nextButton.tap()
    
    // 다음 화면의 특정 레이블 확인
    let newScreenLabel = app.staticTexts["Welcome"]
    XCTAssertTrue(newScreenLabel.exists)  // "Welcome" 텍스트가 나타나는지 확인
}
```

이 UI 테스트는 앱이 Next 버튼을 클릭했을 때 다음 화면에 Welcome 텍스트가 나타나는지를 확인하는 예제입니다.

<br>

### 2. 단위(Unit) 테스트

단위 테스트는 애플리케이션의 개별 구성 요소나 메서드가 올바르게 작동하는지 검증합니다. 특정 함수나 메서드가 예상한 결과를 반환하는지, 로직에 오류가 없는지를 확인하여 코드의 품질을 유지합니다.

- 주요 목적: 개별 함수 또는 메서드가 정상적으로 동작하는지 확인
- 테스트 범위: 클래스나 메서드와 같은 작은 코드 단위
- 실행 환경: 테스트 더블 또는 Mock을 사용해 특정 기능만 독립적으로 검증
- 종류: 기능 테스트, 로직 검증, 입력 값과 출력 값의 일치 확인

#### 예시 코드: Swift에서 단위 테스트 작성

```swift
// 정수 배열에서 최대값을 찾는 함수를 테스트하는 단위 테스트입니다.
func testFindMaximumValue() {
    let numbers = [3, 5, 9, 1, 6]
    let maxValue = findMaximum(numbers)
    XCTAssertEqual(maxValue, 9)  // 최대값이 9인지 확인
}
...
이 단위 테스트는 findMaximum 함수가 정수 배열 [3, 5, 9, 1, 6]에서 올바른 최대값(9)을 반환하는지를 확인합니다.

<br>

### 요약 비교

<img src="https://github.com/user-attachments/assets/ad1bf222-ad10-4dfa-a44b-0d2947664cb5">

<br>

### 결론
- UI 테스트는 사용자 경험의 흐름을 검증하는 데 유용하며, 전체적인 앱의 기능적 일관성을 확인할 수 있습니다.
- 단위 테스트는 개별 구성 요소를 독립적으로 검증하여, 코드의 정확성을 높이고 잠재적 오류를 사전에 방지할 수 있습니다.
...
## 21.2 테스트 주도 개발(TDD)의 장점은 무엇인가요?
**테스트 주도 개발(TDD; Test-Driven Development)** 은 테스트 케이스를 먼저 작성한 후, 테스트를 통과할 수 있도록 코드를 구현해 나가는 개발 방식입니다. TDD는 소프트웨어의 품질을 높이고 유지보수성을 강화하기 위해 널리 사용됩니다.
...
#### 1.	코드 품질 향상:
- TDD는 테스트가 통과하는 최소한의 코드만을 작성하게 하여, 불필요한 코드 작성이 줄어들고 코드의 품질이 향상됩니다.
- 요구사항에 맞는 기능 구현을 반복적으로 검증하면서 코드의 신뢰성을 높일 수 있습니다.

<br>
 
#### 2.	유지보수 용이성:
- 테스트 케이스가 많아질수록 기존 코드에 대한 검증이 쉬워져 유지보수가 용이해집니다.
- 코드 수정이나 리팩토링을 진행할 때 테스트 케이스가 기존 기능을 검증하므로, 수정된 코드가 예상대로 동작하는지 쉽게 확인할 수 있습니다.

<br>

#### 3.	버그 예방:
- TDD는 기능을 구현하기 전 예상되는 동작을 정의하고 테스트하므로, 사전에 버그를 발견하고 해결할 확률이 높아집니다.
- 테스트가 반복 실행되면서, 예상치 못한 오류를 사전에 찾아 수정할 수 있어 소프트웨어 품질이 향상됩니다.

<br>

#### 4.	작은 단위로 작업 관리:
- TDD는 테스트 케이스를 기반으로 작은 단위의 기능을 검증하며 개발하므로, 복잡한 기능을 단계별로 구현할 수 있습니다.
- 큰 기능을 작은 단위로 나누어 개발하고 테스트하므로, 프로젝트 전체를 더 쉽게 관리하고 코드의 가독성을 높일 수 있습니다.

<br>

#### 5.	문서화 역할:
- 테스트 케이스 자체가 코드의 기능과 동작에 대한 설명서 역할을 합니다.
- 다른 개발자가 코드를 읽을 때, 테스트 케이스를 통해 각 기능의 의도와 예상 결과를 빠르게 파악할 수 있습니다.

<br>

### TDD의 장점을 활용한 예시
#### 1. 요구사항 명세를 통한 코드 작성
요구사항: 정수 배열의 최대값을 찾는 기능이 필요하다고 가정합니다.

<br>

#### 2.	테스트 작성
먼저 테스트 케이스를 작성하여 정수 배열에서 최대값을 찾는 기능을 명확히 정의합니다.

```swift
func testFindMaximumValue() {
    let numbers = [1, 3, 7, 2, 5]
    let maxValue = findMaximum(numbers)
    XCTAssertEqual(maxValue, 7)  // 예상 최대값: 7
}
```
...
#### 4.	리팩토링
테스트가 통과하면 코드 구조를 개선하고 필요 없는 부분을 정리할 수 있습니다.

<br>

TDD의 작업 흐름: **“테스트 작성 -> 코드 구현 -> 리팩토링”**의 반복


<br>

TDD는 코드를 작성하기 전부터 명확한 목표와 요구사항을 바탕으로 진행하기 때문에, 결과적으로 더 신뢰할 수 있고 유지보수가 쉬운 코드를 생산하게 됩니다.
