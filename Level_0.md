...
- 서버는 요청에 응답한 후 연결을 끊습니다. 따라서 각 요청이 독립적으로 처리됩니다.

<br>

#### 2. 무상태성(Stateless)
- HTTP는 상태를 유지하지 않는 프로토콜입니다.
- 클라이언트와 서버 간에 한 번의 요청-응답이 이루어지고 나면, 이전 요청에 대한 정보가 유지되지 않습니다.
- 상태를 유지하려면 세션이나 쿠키를 사용해야 합니다.

<br>

#### 3. 유연성
- HTTP는 텍스트, 이미지, 비디오 등 다양한 형태의 데이터를 전송할 수 있는 유연성을 가지고 있으며, MIME 타입을 통해 데이터를 구분합니다.

<br>

#### 4. 확장성
- HTTP 헤더를 통해 다양한 정보(예: 언어, 캐시, 인증 등)를 전송할 수 있어 기능 확장이 용이합니다.

<br>

### HTTP/1.1과 HTTP/2의 차이점

<img width="751" alt="스크린샷 2024-10-31 오후 8 27 14" src="https://github.com/user-attachments/assets/20932789-2c13-4f01-a48c-6b58ec17120a">

#### HTTP/1.1의 특징

- Persistent Connection:
	- HTTP/1.1에서는 한 번의 연결을 통해 **여러 요청을 연속적으로 처리할 수 있는 지속적 연결(Persistent Connection)** 을 도입했습니다.
 	- 연결이 지속되기 때문에 새로운 요청을 위해 매번 연결을 생성하지 않아도 됩니다.
- 파이프라이닝(Pipelining):
	- 클라이언트가 여러 요청을 순차적으로 보내고, 서버가 순서대로 응답할 수 있게 하는 기능
 	- 많은 서버가 파이프라이닝을 완전히 지원하지 않아 사용이 제한적입니다.
...
- 멀티플렉싱(Multiplexing):
	- HTTP/2에서는 하나의 TCP 연결을 통해 여러 요청과 응답을 동시에 전송할 수 있습니다.
 	- 이를 통해 지연 시간이 줄어들고, 성능이 크게 향상됩니다.
- 헤더 압축(HPACK):
	- HTTP/2는 HPACK이라는 헤더 압축 방식을 사용하여 중복된 헤더 정보를 압축해 전송 효율을 높입니다.
- 서버 푸시(Server Push):
	- 서버가 클라이언트가 요청하지 않은 리소스를 미리 전송할 수 있는 기능입니다.
 	- 이를 통해 클라이언트가 필요한 리소스를 미리 로드하여 페이지 로딩 시간을 단축할 수 있습니다.

<br>

#### 요약
- HTTP/1.1:
	- 요청마다 별도의 TCP 연결이 필요하거나 순차적인 처리가 이루어져 속도가 제한적입니다.
- HTTP/2:
	- 하나의 연결에서 멀티플렉싱을 통해 여러 요청과 응답을 동시에 처리할 수 있어 전송 속도가 더 빠르고 효율적입니다.
	- 또한, 헤더 압축과 서버 푸시 기능으로 데이터 전송의 효율성을 높입니다.

<br>

## 13.1 HTTP의 무상태(Stateless) 성질은 무엇이며, 어떻게 극복하나요?
**HTTP의 무상태성(Stateless)** 은 서버가 각 요청 간의 연결 상태를 유지하지 않는 성질을 의미합니다. 즉, HTTP는 클라이언트가 보낸 요청을 처리하고 응답한 후, 서버는 요청에 대한 정보를 유지하지 않고, 다음 요청을 받을 때마다 새로운 요청으로 처리합니다.

<br>

### HTTP의 무상태성의 특징과 문제점
- 각 요청 독립성:
	- HTTP 요청은 서로 독립적으로 처리됩니다.
 	- 클라이언트가 서버에 여러 요청을 보낼 때, 서버는 이전 요청에 대한 정보를 기억하지 않습니다.
- 상태 저장 필요성:
	- 웹 애플리케이션에서 로그인 상태 유지, 장바구니 관리, 사용자 설정 등 상태를 기억해야 할 경우, HTTP의 무상태성은 불편함을 초래할 수 있습니다.
 	- 서버는 각 요청마다 클라이언트의 상태를 기억하지 않기 때문에, 사용자별 상태 정보를 관리하기 어렵습니다.

<br>

### HTTP 무상태성 극복 방법
HTTP의 무상태성을 극복하기 위해, 웹 애플리케이션에서는 클라이언트와 서버 간에 상태를 유지하는 여러 가지 방법을 사용합니다. 대표적인 방법으로 쿠키, 세션, 토큰 기반 인증(JWT) 등을 사용할 수 있습니다.
...
- 개념:
	- 쿠키는 클라이언트의 웹 브라우저에 상태 정보를 저장하는 작은 데이터 조각입니다.
 	- 서버가 클라이언트에게 응답할 때, 쿠키를 함께 전송하여 클라이언트의 브라우저에 저장하도록 요청할 수 있습니다.
- 동작 방식:
	- 이후 클라이언트가 다시 서버에 요청할 때, 해당 쿠키를 요청에 포함하여 보냄으로써 서버는 클라이언트의 상태 정보를 확인할 수 있습니다.
- 활용 예시:
	- 사용자가 로그인한 상태를 유지하거나, 장바구니 데이터를 저장하는 데 활용됩니다.

<br>

#### 2. 세션(Session)
- 개념:
	- 세션은 서버 측에 사용자 상태 정보를 저장하여, 클라이언트가 서버에 연결된 동안 상태를 유지하게 하는 방법입니다.
- 동작 방식:
	- 서버는 클라이언트에게 세션 ID를 발급하고, 클라이언트는 요청 시마다 이 세션 ID를 포함하여 서버에 보냅니다. 서버는 해당 세션 ID를 기반으로 사용자 정보를 조회하여 상태를 유지합니다.
- 활용 예시:
	- 세션은 주로 로그인 상태 유지나 쇼핑몰의 장바구니 기능 등 사용자별 상태를 관리하는 데 사용됩니다.



<br>

#### 3. 토큰 기반 인증 (JWT - JSON Web Token)

- 개념:
	- JWT는 사용자 정보를 JSON 형식으로 인코딩한 토큰을 사용하여 인증 및 상태 유지를 지원합니다.
 	-  JWT는 클라이언트 측에서 관리되며, 서버는 별도의 세션 저장소가 필요하지 않습니다.
- 동작 방식:
	- 클라이언트가 로그인 시 인증 정보를 포함한 JWT를 서버로부터 발급받고, 이후 요청 시마다 JWT를 포함하여 서버에 전송합니다.
 	- 서버는 JWT의 유효성을 검증하여 클라이언트의 상태를 확인합니다.
- 활용 예시:
	- JWT는 주로 RESTful API의 인증에 사용되며, 서버의 확장성과 성능을 높이는 데 유리합니다.


<br>

### 비교 요약
<img src="https://github.com/user-attachments/assets/dfd46623-9f6b-4acc-95a1-e9a44cd05537">


<br>

### 요약
- HTTP의 무상태성은 요청 간 상태를 유지하지 않는 특성으로, 사용자 상태를 관리하기 어려운 단점이 있습니다.
- 이를 극복하기 위해 쿠키, 세션, JWT 등을 사용하여 상태 정보를 유지하며, 각각의 방법은 상황과 목적에 맞게 선택됩니다.
...
<img src="https://github.com/user-attachments/assets/1bdffbf3-35df-4f2f-859e-c03c6601139f">

### 결론
HTTP/2는 멀티플렉싱, 헤더 압축, 서버 푸시, 스트림 우선순위, 이진 프로토콜과 같은 기능을 통해 HTTP/1.1의 단점을 개선하고 웹 성능을 최적화했습니다. 이를 통해 페이지 로딩 속도를 크게 향상시키고, 사용자 경험을 개선하는 데 기여하고 있습니다.
...
<img src="https://github.com/user-attachments/assets/3c47a93a-d56c-45f7-a605-3ca9b42ef526">

### 결론
HTTP/3는 UDP 기반 QUIC 프로토콜, 0-RTT 연결 설정, 헤드 오브 라인 블로킹 해결, TLS 1.3, 다중화 기능을 통해 기존 HTTP/2보다 속도와 안정성, 보안성이 더욱 향상되었습니다. 이러한 기능 덕분에 HTTP/3는 고속 네트워크 환경과 실시간 애플리케이션에 더욱 적합하며, 웹 성능을 한층 더 개선할 수 있습니다.
...
<img src="https://github.com/user-attachments/assets/f3a4720e-dba2-42e0-9c26-8f0a3515308a">

### 요약

- 연결 지향형 프로토콜(TCP)은 데이터의 신뢰성, 순서, 무손실 전송을 보장하며, 연결 설정과 확인 응답 과정을 거쳐 전송됩니다. 주로 파일 전송이나 웹 브라우징에 사용됩니다.
- 비연결 지향형 프로토콜(UDP)은 빠른 전송 속도를 위해 연결 설정 없이 데이터를 전송하며, 신뢰성이 보장되지 않습니다. 실시간 전송이 필요한 스트리밍이나 온라인 게임에 적합합니다.
...
- 클라이언트는 서버와의 연결을 요청하기 위해 SYN 패킷을 서버로 보냅니다.
- 이 패킷에는 **초기 순서 번호(ISN, Initial Sequence Number)** 가 포함되어 있으며, 이 번호를 기준으로 이후 데이터 전송의 순서가 정해집니다.
- 상태 변화:
	- 클라이언트는 SYN 패킷을 전송하고 SYN-SENT 상태로 진입합니다.

 <br>
 
#### 2.	SYN-ACK (Synchronize-Acknowledgment) - 요청 수락 및 응답
- 서버는 클라이언트의 요청을 받아들이고, 클라이언트로부터 받은 SYN에 대한 **승인 응답(ACK)** 과 함께 자신의 SYN 패킷을 동시에 전송합니다.
- 서버는 클라이언트의 초기 순서 번호에 1을 더한 ACK 번호와 자신의 초기 순서 번호(ISN)를 설정하여, 이후 데이터 통신에서 사용할 준비를 합니다.
- 상태 변화:
	- 서버는 SYN-ACK 패킷을 전송한 후 SYN-RECEIVED 상태로 진입합니다.

<br>

#### 3.	ACK (Acknowledgment) - 연결 수락 완료
- 클라이언트는 서버의 응답을 받고, 서버의 SYN에 대한 **확인 응답(ACK)** 을 다시 서버로 보냅니다.
- 이 과정이 완료되면 클라이언트와 서버 간에 연결이 성립되며, 양측 모두 데이터 전송을 할 준비가 완료됩니다.
- 상태 변화:
	- 클라이언트는 ACK 패킷을 전송하고 ESTABLISHED 상태로 진입하며, 서버도 클라이언트의 ACK를 수신하고 ESTABLISHED 상태로 진입합니다.

<br>

### 요약

<img src="https://github.com/user-attachments/assets/2a3df069-e48b-4385-85da-e0b52606a729">

TCP의 3-Way Handshake는 클라이언트와 서버 간의 안정적인 연결을 설정하기 위한 필수 과정입니다. SYN → SYN-ACK → ACK의 세 단계를 거쳐 연결을 설정하며, 이 과정을 통해 데이터 전송 준비가 완료됨을 상호 확인하고 신뢰성 있는 데이터 전송이 가능해집니다.
...
### UDP 사용이 적합한 이유 요약
	...
	• 빠른 전송 속도가 요구되는 상황
	• 데이터 신뢰성보다는 실시간성이 중요한 경우
	• 약간의 패킷 손실을 허용할 수 있는 애플리케이션
...
<img src="https://github.com/user-attachments/assets/9957e92a-f8e6-4e4b-aa91-c79db7f6d53d">

### iOS에서 포맷 선택 기준
- UI 이미지: PNG (높은 품질과 투명도 지원)
- 사진: HEIC (높은 품질과 효율적인 용량), JPEG (호환성을 위해 사용 가능)
- 애니메이션: GIF (간단한 움직임 표현)
...
#### File signature
- PNG 파일의 시작을 나타내는 8바이트 고정 코드로, PNG 파일임을 식별하는 역할을 합니다.
- [ 89 50 4E 47 0D 0A 1A 0A ]
- 이중 50 4E 47이 ASCII 값으로 PNG이기도 해서, Hex viewer에서 PNG 파일을 열었을 때 이 부분이 보이면 쉽게 PNG 파일이라고 판단할 수 있습니다.


<br>

#### Chunks
PNG 파일은 여러 **청크(chunk)**로 구성되며, 각 청크는 이미지의 특정 정보를 담고 있습니다. 청크의 구조는 아래와 같습니다.
```
{
...
#### IHDR 
- IHDR 청크는 파일의 첫 번째 청크로 반드시 존재
- 이미지의 기본 정보(가로·세로 크기, 색상 타입 등)를 저장
- 구성 정보:
	- 가로, 세로 크기: 이미지의 너비와 높이(픽셀 단위)입니다.
- 비트 깊이(Bit Depth):
	- 각 채널당 비트 수를 정의하며, 이미지의 색상 정밀도를 결정합니다.
 	- 예: 1, 2, 4, 8, 16비트.
- 색상 타입(Color Type):
	- 이미지가 사용하는 색상 포맷을 정의합니다.
	- 예: 그레이스케일, RGB, 팔레트 색상, 그레이스케일+알파, RGBA.
- 압축 방법:
	- PNG 표준은 zlib 무손실 압축을 사용합니다.
- 필터 방법:
	- 데이터 필터링에 사용되는 알고리즘을 정의합니다.
- 인터레이싱(Interlacing):
	- 이미지가 점진적으로 로드될지 여부를 결정합니다(Adam7 인터레이싱 사용).
- 특징:
	- IHDR 청크는 파일의 첫 번째 청크로 반드시 존재하며, PNG 이미지의 기본적인 구조와 속성을 정의합니다.
```
{
 Length : 00 00 00 0D (13 byte),
 Chunk Type : IHDR,
 Chunk Data ( 13 byte ), // 언제나 13바이트
 {
   Width (4 byte),
   Height (4 byte),
   Bit depth (1 byte),
...
#### PLTE (Palette)
- 역할: 팔레트 기반 색상 정보를 저장하는 청크로, 색상표를 정의하여 저장 공간을 절약할 수 있습니다.
- 구성 정보:
	- 최대 256개의 색상 값을 저장하며, 각 색상은 RGB 값으로 구성됩니다.
	- 색상당 8비트를 사용하여 총 256가지 색상을 정의할 수 있습니다.
- 특징:
	- PLTE 청크는 팔레트 색상 모드(예: GIF와 같은 색상 팔레트 기반 이미지)에서만 사용되며, RGB 또는 그레이스케일 이미지에는 포함되지 않습니다.
	- PLTE가 사용될 경우, 각 픽셀은 팔레트의 색상 인덱스로 저장되어 데이터 크기를 줄이는 역할을 합니다.
...
#### IDAT (Image Data)
- 역할:
	- 실제 이미지 데이터를 저장하는 청크입니다.
- 구성 정보:
	- IDAT 청크에는 압축된 이미지 데이터가 포함됩니다.
	- 여러 개의 IDAT 청크가 연속으로 이어질 수 있으며, 이는 데이터를 블록 단위로 나누어 저장하여 더 큰 이미지를 처리할 수 있게 합니다.
	- 데이터는 zlib 무손실 압축 방식으로 압축되어 저장되며, 이 압축 방식은 이미지의 품질을 유지하면서도 파일 크기를 줄이는 데 사용됩니다.
- 특징:
	- 하나 이상의 IDAT 청크가 연속으로 포함될 수 있으며, PNG 파일의 주요 데이터 부분을 차지합니다.
 	- 일반적으로 한 IDAT 청크당 65536 바이트의 데이터 크기를 갖습니다.
	- IDAT 청크가 모든 이미지 데이터를 포함하기 때문에, 데이터 손실 없이 이미지 원본을 복원할 수 있습니다.
	- 착각하기 쉬운 것은, 하나의 IDAT 청크가 이미지의 특정 부분을 나타내지는 않는다는 점입니다. PNG는 전체 이미지 데이터를 한꺼번에 압축한 뒤, 여러 IDAT 청크에 나누어 담는 방식을 사용합니다. 따라서, 모든 IDAT 청크가 있어야만 이미지 디코딩이 가능합니다. 반대로 말하면, 하나의 IDAT 청크라도 사라지면 압축된 이미지를 디코딩 할 수 없습니다.
...
#### 필터링
PNG 파일에서 필터링은 데이터를 압축하기 전에 이미지 데이터를 단순화하여 압축 효율을 높이기 위한 전처리 과정입니다. 필터링은 각 픽셀 값이 주변 픽셀과의 차이값으로 저장되게 하여 데이터 패턴을 단순화하고, 그 결과로 zlib 압축을 더 효과적으로 수행할 수 있게 합니다.

<br>

#### IEND (Image End)
- 역할:
	- PNG 파일의 종료를 나타내는 청크로, 파일이 끝났음을 알립니다.
- 구성 정보:
	- 데이터가 포함되지 않는 빈 청크입니다. 청크 헤더와 CRC 필드만 포함됩니다.
- 특징:
	- PNG 파일의 마지막 청크로 위치하며, 이 청크 이후에는 더 이상 데이터가 없습니다.
	- IEND 청크가 나타나면 파일이 끝났음을 나타내며, 이 청크가 없다면 PNG 파일이 올바르게 종료되지 않은 것입니다.
   
<br>

### 요약
<img src="https://github.com/user-attachments/assets/d9cf62a7-d2df-450e-9d66-6f6e582b3e44">

<br>

### PNG 파일의 저장 방식
- 무손실 압축:
	- PNG는 zlib 알고리즘을 사용하여 이미지를 압축합니다. 이 방식은 이미지 품질을 유지하면서 파일 크기를 줄이는 무손실 압축 방식입니다.
- 필터링:
	- 이미지 데이터를 저장하기 전에 필터링을 적용해 데이터 패턴을 단순화하여 압축 효율을 높입니다.
- 인터레이싱(선택적):
	- Adam7 인터레이싱을 사용하여 이미지가 점진적으로 로드되도록 할 수 있으며, 이는 저속 연결 환경에서 유용합니다.
...
#### 1. 메모리 종류
- 스택 메모리:
	- 함수 호출 시 임시로 사용하는 메모리 공간입니다. 변수가 함수 내에서 선언되면 스택에 저장되고 함수가 끝나면 자동으로 해제됩니다.
 	- 메모리 할당과 해제가 빠르지만 용량이 제한적입니다.
- 힙 메모리:
	- 동적으로 할당되는 메모리 영역입니다.
 	- 객체나 데이터 구조를 생성할 때 주로 사용하며, 개발자가 직접 해제해야 하는 메모리입니다.
  	- ARC(Automatic Reference Counting) 방식으로 자동 관리됩니다.
- 가상 메모리:
	- iOS는 물리적 메모리를 넘어서 가상 메모리를 제공합니다.
 	- 가상 메모리를 통해 앱이 실제 메모리 용량을 초과하여 메모리를 사용하는 것을 방지할 수 있습니다.
  	- 그러나 iOS는 가상 메모리 스왑을 지원하지 않기 때문에 실제 메모리 사용량 관리가 중요합니다.

<br>

 
#### 2.	ARC (Automatic Reference Counting)
- iOS는 ARC를 통해 객체의 참조 카운트를 자동으로 관리합니다.
- 참조가 없어지면 메모리가 자동으로 해제되므로 메모리 관리가 용이합니다.
- **순환 참조(Strong Reference Cycle)** 에 유의해야 하며, 이를 방지하기 위해 weak나 unowned 같은 약한 참조를 사용하는 것이 중요합니다.

<br>

#### 3.	메모리 경고(Memory Warning)
- iOS는 앱이 지나치게 메모리를 많이 사용하면 메모리 경고를 발생시킵니다.
- UIApplicationDelegate의 applicationDidReceiveMemoryWarning 메서드나 UIViewController의 didReceiveMemoryWarning 메서드를 통해 메모리 경고를 감지할 수 있으며, 불필요한 리소스를 해제해 메모리 사용량을 줄일 수 있습니다.
...
#### 1. 이미지 및 리소스 관리
- 이미지 크기 최적화:
	- 고해상도 이미지 사용 시 메모리 사용량이 급격히 증가하므로, 필요 이상으로 큰 이미지는 줄이거나 압축하여 사용하는 것이 좋습니다.
- 이미지 캐싱:
	- 자주 사용하는 이미지는 캐싱하여 메모리 로드를 줄이고, 필요하지 않은 이미지는 메모리에서 제거하여 캐싱 메모리를 관리하는 것이 중요합니다.
- Lazy Loading:
	- 메모리를 절약하기 위해 필요한 시점에서만 이미지를 로드하여 사용합니다.

<br>

#### 2.	큰 데이터 구조와 컬렉션 관리
- 대용량 데이터:
	- 대용량 데이터는 필요 시에만 메모리에 로드하며, 사용 후에는 해제하여 메모리 누수를 방지합니다.
- 컬렉션의 메모리 사용량 관리:
	- 큰 배열이나 딕셔너리는 메모리 사용량이 크기 때문에, 사용하지 않는 데이터를 제거하거나 중복 데이터를 줄여 최적화합니다.

<br>

#### 3.	View와 ViewController 메모리 관리
- 뷰 컨트롤러의 메모리 해제:
	- 뷰 컨트롤러가 화면에서 사라지면 불필요한 뷰와 데이터를 메모리에서 해제해야 합니다.
- 뷰의 weak 참조 사용:
	- 뷰나 서브뷰를 강한 참조 대신 약한 참조로 설정하여 뷰 계층에서 메모리 누수를 방지합니다.

<br>

#### 4.	비동기 작업 및 클로저의 메모리 관리
- 비동기 작업:
	- 클로저 내부에서 self를 참조하면 순환 참조가 발생할 수 있습니다. [weak self] 캡처 리스트를 사용하여 이를 방지합니다.
- DispatchQueue:
	- 비동기 작업에서 강한 참조가 유지되지 않도록 주의하며, 작업이 완료되면 메모리를 해제해야 합니다.

<br>

#### 5.	메모리 프로파일링과 최적화
- Xcode의 Instruments 도구를 사용하여 메모리 프로파일링을 통해 메모리 사용량을 측정하고 최적화할 수 있습니다.
- Allocations:
	- 현재 메모리 할당 상황을 모니터링합니다.
- Leaks:
	- 메모리 누수가 발생하는 지점을 확인하여 수정합니다.
- Activity Monitor:
	- 앱이 사용하는 메모리와 CPU 사용량을 모니터링하여 최적화 포인트를 찾습니다.
...
### 메모리 정렬(alignment)
#### 정의: 
- 메모리 정렬은 특정 데이터 타입이 메모리에 저장될 때, 그 주소가 특정 기준(예: 4바이트, 8바이트 등)의 배수로 정렬되는 것을 의미합니다. 
- 예를 들어, 4바이트 정수는 4의 배수 주소에 저장되어야 합니다.

> 패딩의 필요성 :
> - 만약 큰 데이터 타입이 중간에 위치하면, 그 이전의 작은 데이터 타입이 채우지 못한 공간을 맞추기 위해 **빈 공간(패딩)** 이 추가됩니다. 이렇게 해서 전체 구조체가 정렬 기준에 맞춰지게 됩니다.

> 패딩이 생기는 이유 :
> - **패딩(padding)** 이 생기는 이유는 **메모리 정렬(Memory Alignment)** 을 맞추기 위해서입니다. CPU가 데이터를 빠르게 읽고 쓸 수 있도록, 특정 데이터 타입이 메모리에서 정해진 크기(예: 4바이트, 8바이트)의 배수 위치에 저장되도록 요구되기 때문입니다. 이 정렬을 맞추기 위해 메모리 사이에 빈 공간이 추가되는 것이 패딩입니다. 

<br>

#### 성능 영향:
- CPU 접근 효율성:
	- 정렬된 데이터는 CPU가 더 빠르게 접근할 수 있도록 하여 성능을 향상시킵니다.
 	- 비정렬된 데이터는 CPU가 두 번의 메모리 액세스를 필요로 할 수 있습니다.
- 캐시 활용:
	- 정렬된 데이터는 CPU 캐시의 효율적인 사용을 가능하게 합니다.
 	- 캐시 라인이 정렬된 데이터로 구성되면, 캐시 히트율이 증가하여 성능이 개선됩니다.

<br>

### 메모리 정렬이 성능에 미치는 영향
- 비정렬 데이터의 비용:
	- 비정렬된 데이터는 메모리 접근을 비효율적으로 만들어 CPU의 성능 저하를 초래할 수 있습니다.
 	- 특히, 큰 데이터 구조체나 배열에서 그 영향이 더욱 두드러집니다.
- 최적화:
	- 개발자는 데이터 구조체를 정의할 때, 메모리 워드 크기와 정렬을 고려하여 성능을 최적화해야 합니다.
 	- 이를 통해 불필요한 메모리 액세스 횟수를 줄이고, 전반적인 애플리케이션 성능을 향상시킬 수 있습니다.
...
#### 1. 고정된 메모리 용량:
- iOS 디바이스는 모델마다 정해진 고정 메모리 용량을 가지고 있으며, 업그레이드할 수 없습니다.
- 예를 들어 최신 iPhone 모델에는 4GB6GB의 메모리가 있지만, 구형 모델에는 1GB2GB에 불과한 경우가 많습니다. 따라서 같은 앱이라도 사용 가능한 메모리 양은 기기마다 차이가 납니다.

<br>

#### 2. 가상 메모리 스왑 미지원:
- macOS와 달리 iOS는 디스크 스왑을 통한 가상 메모리 지원이 없습니다.
- 즉, 앱이 물리적 메모리를 초과해 사용할 수 없으며, 메모리 부족 시 시스템은 백그라운드 앱을 종료하거나, 해당 앱을 종료할 수 있습니다.

<br>

### 앱 메모리 제한
#### 1. 앱당 메모리 할당 한계:
- 각 앱은 기기의 메모리 용량에 따라 할당 가능한 메모리에 제한이 있습니다.
- 최신 고사양 기기에서는 앱이 사용할 수 있는 메모리 용량이 더 많지만, 저사양 기기나 구형 기기에서는 제한이 더 낮습니다.
- 이 제한을 초과하면 시스템에서 메모리 부족으로 인해 앱을 강제 종료(Out-Of-Memory, OOM)할 수 있습니다.

<br>

#### 2.	메모리 경고:
- 앱이 메모리 한계에 도달할 경우, iOS는 applicationDidReceiveMemoryWarning이나 UIViewController의 didReceiveMemoryWarning 메서드를 호출하여 메모리 경고를 보냅니다.
- 이 경고를 처리하지 않으면 메모리가 해제되지 않고 계속 증가하게 되어, iOS가 앱을 강제로 종료할 수 있습니다.
- 불필요한 데이터와 리소스를 해제하여 메모리를 확보하는 것이 중요합니다.

<br>

#### 3.	백그라운드 앱과의 메모리 경쟁:
- iOS는 멀티태스킹을 지원하지만, 백그라운드 앱은 메모리 사용이 제한됩니다. 포그라운드에서 실행 중인 앱이 메모리 경고를 받을 때, iOS는 백그라운드 앱을 종료해 메모리를 확보하기도 합니다.
- 백그라운드로 전환되는 앱은 상태를 저장해 필요 시 복원할 준비가 되어 있어야 합니다.
...
#### 1.메모리 프로파일링: 
- Xcode의 Instruments를 사용해 메모리 사용량을 모니터링하고, 메모리 누수와 불필요한 사용을 줄입니다.
#### 2.	리소스 해제와 캐싱 최적화: 
- 메모리 경고가 발생하면 캐시 데이터를 해제하여 메모리를 확보합니다. 불필요한 이미지를 해제하거나 다시 로드하여 메모리를 절약합니다.
#### 3.	대용량 데이터와 이미지 처리 최적화: 
- 이미지와 비디오 같은 대용량 데이터는 필요한 시점에만 로드하고, 사용이 끝난 후 메모리에서 해제합니다.
#### 4.	비동기 처리: 
- 백그라운드 스레드에서 작업을 수행하여 메모리 사용량을 최적화합니다. 클로저에서 weak 또는 unowned 참조를 사용해 순환 참조를 방지합니다.

<br>
<br>

## 22.1 메모리 경고(Memory Warning)가 발생하면 어떤 조치를 취해야 하나요?
### 메모리 경고 발생 시 취해야 할 조치
#### 1. 캐시 데이터 해제
- 이미지, 데이터 캐시 등 다시 불러올 수 있는 데이터는 해제하여 메모리 사용량을 줄입니다.
- 예를 들어, NSCache를 사용해 관리하던 이미지 캐시를 비우거나, 이미지 데이터를 캐시 대신 디스크에 저장하여 필요할 때 다시 불러옵니다.

<br>

#### 2.	불필요한 리소스 해제
- 화면에 표시되지 않는 뷰, 뷰 컨트롤러, 데이터 모델 등 필요하지 않은 객체를 메모리에서 해제합니다.
- 예를 들어, 화면에 표시되지 않는 뷰나, 백그라운드 데이터 로딩 작업이 중단된 경우, 해당 리소스를 해제합니다.

<br>

#### 3.	Lazy Loading 사용
- 필요한 시점에 데이터를 로드하여 메모리를 절약하는 Lazy Loading 기법을 활용합니다.
- 큰 이미지나 데이터는 화면에 실제로 나타날 때 로드하고, 화면에서 사라지면 메모리에서 해제합니다.

<br>

#### 4. 이미지 및 파일 처리 최적화
- 고화질 이미지나 비디오 같은 대용량 파일은 메모리 경고 시 해제합니다.
- 예를 들어, 대형 이미지를 UIImageView에 직접 할당하지 않고, 압축된 이미지 버전이나 낮은 해상도 버전을 사용하여 메모리를 절약합니다.

<br>

#### 5.	백그라운드 태스크 중지
- 메모리 경고가 발생하면 백그라운드 작업을 중지하여 메모리 사용을 줄입니다.
- 예를 들어, 데이터 다운로드, 네트워크 요청과 같은 백그라운드 작업이 많은 메모리를 차지하고 있는 경우, 이를 중단하거나 일시 중지합니다.

<br>

#### 6.	UIKit에서 제공하는 메모리 경고 메서드 활용
- applicationDidReceiveMemoryWarning: 전체 앱이 메모리 경고를 받을 때 호출됩니다. 이 메서드를 활용해 앱 전반의 리소스 정리 작업을 수행할 수 있습니다.
- UIViewController의 didReceiveMemoryWarning: 각 뷰 컨트롤러에서 메모리 경고가 발생하면 호출됩니다. 뷰 컨트롤러 내에서 사용하지 않는 리소스 해제에 활용할 수 있습니다.
```swift
...
#### 7.	메모리 프로파일링 및 최적화
- Xcode의 Instruments를 사용하여 메모리 사용 패턴을 분석하고, 불필요한 메모리 사용이나 누수를 확인하여 최적화합니다.
- Instruments의 Allocations 도구를 사용해 메모리 사용량을 모니터링하고, Leaks 도구를 사용해 메모리 누수가 발생하는 부분을 수정합니다.
...
### 동적 프로그래밍의 핵심 개념
1. 최적 부분 구조(Optimal Substructure): 큰 문제의 최적해가 작은 부분 문제들의 최적해로 구성될 수 있어야 합니다.
2. 중복되는 부분 문제(Overlapping Subproblems): 동일한 부분 문제가 여러 번 반복되어 나타나야 합니다.
...
피보나치 수열은 DP의 대표적인 예시입니다.

피보나치 수열의 정의는:
- 기저 조건 :  F(0) = 0 ,  F(1) = 1 
- 재귀 정의 :  F(n) = F(n-1) + F(n-2) 

#### 1. Top-Down 방식 (재귀적 + 메모이제이션)
```swift
var memo = [Int: Int]()
...
### 동적 프로그래밍의 장점
- 효율성: 중복되는 부분 문제를 저장하고 재사용하여 계산 시간을 크게 줄일 수 있습니다.
- 문제 해결 범위 확장: 단순 재귀로는 해결하기 어려운 문제도 DP로 효율적으로 해결할 수 있습니다.
...
- 최소 비용 경로 찾기 (예: Floyd-Warshall 알고리즘)

<br>

### 요약
동적 프로그래밍은 중복 계산을 피하고 최적화된 해를 찾기 위해 부분 문제의 결과를 저장해두는 방법입니다. 특히 최적화 문제에서 매우 유용하게 활용되며, Top-Down과 Bottom-Up 접근 방식을 통해 문제를 효율적으로 해결할 수 있습니다.
...
**가상 메모리(Virtual Memory)** 는 컴퓨터 시스템에서 물리적 메모리(RAM)의 크기와 상관없이 더 큰 메모리 공간을 사용할 수 있게 하는 메모리 관리 기법입니다. 가상 메모리는 물리적 메모리와 디스크 공간을 조합하여, 실행 중인 프로그램이 필요로 하는 메모리를 유연하게 관리하고 확장할 수 있게 합니다.

### 가상 메모리의 개념
가상 메모리는 프로세스가 사용하는 메모리 주소를 가상 주소로 관리하고, 실제 물리적 메모리(RAM)와 하드 디스크(저장 장치)에 데이터를 분할하여 저장함으로써, 물리 메모리의 한계를 넘어서 프로그램이 실행될 수 있도록 합니다.

### 가상 메모리의 주요 목적
1. 메모리 확장: 실제 물리적 메모리보다 더 큰 용량의 메모리를 사용할 수 있게 합니다.
2. 프로세스 격리: 각 프로세스마다 독립된 가상 메모리 공간을 제공하여, 하나의 프로세스가 다른 프로세스의 메모리에 접근하지 못하도록 보호합니다.
3. 메모리 보호: 운영체제가 메모리 접근을 관리함으로써, 잘못된 메모리 접근으로부터 시스템을 보호합니다.

<br>

### 가상 메모리의 동작 원리
가상 메모리는 **페이징(Paging)**이라는 기법을 사용해 물리적 메모리와 디스크 간의 데이터를 이동시킵니다.

1. 페이지와 페이지 프레임:
- 페이지(Page): 가상 메모리를 동일한 크기의 블록으로 나눈 것입니다.
- 페이지 프레임(Page Frame): 물리 메모리(RAM)를 페이지와 동일한 크기의 블록으로 나눈 것입니다.

2. 페이지 테이블(Page Table):
- 페이지 테이블은 가상 주소와 물리 주소 간의 매핑 정보를 저장하는 테이블입니다.
- 운영체제는 각 프로세스마다 페이지 테이블을 유지하고, 이를 통해 가상 주소를 물리 주소로 변환하여 실제 메모리에 접근합니다.

  
3. 페이지 부재(Page Fault):
- 프로세스가 필요로 하는 페이지가 물리 메모리에 없는 경우 페이지 부재가 발생합니다.
- 페이지 부재가 발생하면 운영체제는 해당 페이지를 디스크에서 물리 메모리로 불러오고, 이를 페이지 테이블에 업데이트하여 다시 실행합니다.


4. 스왑(Swap)과 페이지 교체:
- 물리 메모리에 빈 공간이 없으면, 운영체제는 스왑 공간(Swap Space) 또는 페이지 교체 알고리즘을 사용해 필요하지 않은 페이지를 디스크로 옮기고, 필요한 페이지를 메모리로 불러옵니다.
- 대표적인 페이지 교체 알고리즘에는 LRU(Least Recently Used), FIFO(First In, First Out) 등이 있습니다.

<br>

### 가상 메모리의 동작 과정 요약
1. 프로세스가 가상 메모리 주소에 접근합니다.
2. 운영체제는 페이지 테이블을 확인하여 가상 주소를 물리 주소로 변환합니다.
3. **필요한 페이지가 물리 메모리에 없다면 페이지 부재(Page Fault)** 가 발생하고, 디스크에서 해당 페이지를 메모리로 불러옵니다.
4. 필요할 때마다 디스크와 메모리 간의 스왑이 이루어지며, 메모리 관리가 수행됩니다.

<br>

### 장점:
- 메모리 용량의 한계 극복: 실제 메모리보다 큰 주소 공간을 사용할 수 있습니다.
- 효율적 메모리 관리: 프로세스가 실제 필요한 메모리만 로드하므로 메모리 활용도가 높아집니다.
- 프로세스 격리 및 보호: 각 프로세스가 독립된 메모리 공간을 가지므로 보안이 강화됩니다.

### 단점:
- 디스크 접근 속도의 한계: 페이지 부재가 빈번하게 발생하면 디스크 I/O가 증가하여 속도가 느려질 수 있습니다.
- 오버헤드: 페이지 테이블 관리 및 주소 변환에 추가적인 오버헤드가 발생합니다.

<br>

### iOS에서의 가상 메모리

iOS에서도 가상 메모리를 사용하여 프로세스를 격리하고 메모리 공간을 효율적으로 관리하지만, 모바일 환경에서는 메모리 용량이 상대적으로 적기 때문에 메모리 관리가 더욱 중요하게 다뤄집니다. iOS에서는 메모리 부족 시 메모리 경고를 발생시키고, 메모리를 과다하게 사용하는 앱을 종료하는 등의 방식으로 메모리 관리를 수행합니다.
